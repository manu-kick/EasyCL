...
def intrinsic_dimension_mle(cf, text_embeddings, audio_embeddings, vision_embeddings, iterations, labels,
    ...
    Args:
        text_embeddings/audio_embeddings/vision_embeddings:
    ...
    modalities_embeddings = {
        ...
        "audio": torch.as_tensor(audio_embeddings, dtype=torch.float32, device="cpu"),
    ...
    X_audio = modalities_embeddings["audio"].numpy()  # (N, D)
    X_vision= modalities_embeddings["vision"].numpy() # (N, D)
    X_all   = np.concatenate([X_text, X_audio, X_vision], axis=0)  # (3N, D)
    ...
    for name, X in [("text", X_text), ("audio", X_audio), ("vision", X_vision), ("all", X_all)]:
    ...
    if cf.wandb and iterations%100 == 0:
        ...
        wandb.log({f"intrinsic_dimension_audio": float(out['audio']['id_global'])})
